...
public:
    void startVisualization(vector<int> shortest_path, string txtFile, string fontPath) {
        ...
        /*Vector2f rawSource(4427.32f, 1330.61f), rawDest(5440.53f, 8567.77f);
        ...
        while (window.isOpen()) {
            ...
            //int first = shortest_path.front();
            //int last = shortest_path.back();
            //drawLine(window, sourcePos, Vector2f(nodes[first].x, nodes[first].y), Color::Green); //line mben source w first shortest path node
            //drawLine(window, Vector2f(nodes[last].x, nodes[last].y), destinationPos, Color::Green); //line mben last shortest path node w destination
